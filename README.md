...
**视图（Element）**
- 递归检视当前 App 的 UIView 层级树，逐节点展示类名 / frame / alpha / hidden / tag / 子视图数
- 点击展开/折叠子树、下拉刷新重新扫描；自动过滤 vConsole 自身浮层窗口，只呈现业务视图

**性能（Performance）**
- `CADisplayLink` 实时 FPS + 基于 `mach` 的 App CPU 占用与常驻内存，FPS 折线 sparkline + 颜色分级（≥50 绿 / 30~50 橙 / <30 红）
- 仅在面板可见时采样，切走即停，零常驻开销
